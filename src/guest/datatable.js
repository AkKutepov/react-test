...
export default class extends React.Component {
  ...
  constructor(props) {
    ...
    this.state.OBJ.hash = window.location.hash
    ...
    else this.state.AR.data = []
    
    this.state.AR.numbers = ['rank', 'supply']
  }
  ...
  methods = {
    ...
    dataColumns() {
      var i = 0
      this.state.AR.columns = []
      for(let key in this.state.AR.data[0]) {
        this.state.AR.columns[i] = { column: key.at(0).toUpperCase() + key.slice(1) }
        if(++i >= 5) break
      }
      // 0 <-> 1 (rank <-> id)
    ...
    getOnClick() {
      ...
      var viewData = (res) => {
        ...
        if(this.state.AR.hdata.waiting) {
          this.state.AR.hdata.waiting = 0
          this.state.AR.hdata.ready = 1
    ...
    headOnClick() {
      ...
      const getValue = (x, colName) => {
        return this.state.AR.numbers.find(item => item == colName) ? +x[colName] : x[colName]
      }
      
      // sort direction
      if(this.state.OBJ.sortColName != colName) {
        this.state.OBJ.sortColName = colName
        this.state.OBJ.sortDirection = 1
      }
      else this.state.OBJ.sortDirection = [0, 1, -1][this.state.OBJ.sortDirection + 1]
      
      this.state.AR.hdata.notransition = 1

      this.state.AR.dataSort = !this.state.OBJ.sortDirection 
    ...
    setArrow(target) {
      ...
      if(this.state.OBJ.sortDirection == 1) arrow = 'my-datatable_arrow_asc'
      else if(this.state.OBJ.sortDirection == -1) arrow = 'my-datatable_arrow_desc'
    ...
    binOnClick() {
      ...
      if(~(i = this.state.AR.dataSort.findIndex(item => item.rank == tdKey.innerText))) {
